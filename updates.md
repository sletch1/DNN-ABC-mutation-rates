...
`Models/1D/matlab/` and `Models/3D/matlab/`), `RCode/`, and `DNN_Prototypes/`: **no occurrence of `p1`, `p2`, or `tau` anywhere.** Every simulator is single-$p$ — `mut_bMBP_rev.m`, `funMBP.R`, `simulator.py` all share the signature `(Z0, a, delta, p, tp)`.
