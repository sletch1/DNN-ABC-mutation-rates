...
def mut_bmbp_fast(Z0, a, delta, p, tp, rng: np.random.Generator):
    ...
    Only valid in the large-Z0/large-Z regime where the exact discrete
    process is well approximated by these continuous shortcuts -- see
    `mut_bmbp_slow` for the exact alternative used as ground truth.
    """
    Z0 = int(Z0)
