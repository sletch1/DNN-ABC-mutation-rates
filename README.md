...
> ### ⚠️ The 3-D study is being rebuilt — §9 and its tables are superseded
>
...
>
> The rebuild targets the paper's actual model, **`(p1, p2, τ)`**, using the
> reference MATLAB in [`DNN_Models/3D/matlab/`](DNN_Models/3D/matlab/). Ground
> truth is generated by [`RCode/genSlowData_3D.R`](RCode/genSlowData_3D.R).
>
> **§9 below, and every 3-D table and figure it cites, describes the retired
> study.** They are kept only for provenance until the new study replaces them.
> The 1-D study (§1–§8) is unaffected — it matches the paper's Study 1 and its
> constant-rate assumptions hold.
...
- ~~A novel **3-D joint `(p, a, δ)` surrogate** (§9)~~ — **retired**, see the notice
  above. Replaced by an in-progress two-stage `(p1, p2, τ)` study on the paper's
  actual multi-parameter model; ground truth is generated and in the repo.
...
- **Dimensionality (target #3) — achieved.** The 3-D `(p, a, δ)` surrogate is built
  and tested (§9 — retired study): a residual
...
├── MatlabCode/     The professor's original MATLAB: the constant-rate simulator,
│                   the MOM/MLE baselines, and the ABC-MCMC / GPS-ABC sampler
│                   this project reimplements and benchmarks against.
├── DNN_Models/     Python: the neural surrogates and the ABC pipelines built
│                   on them. 1D/ is complete; 3D/ is being rebuilt.
