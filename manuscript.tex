...
First, a curve-fit sweep from the deployed size down to a linear control,
...
\textbf{32--16} & \textbf{626} & $\mathbf{3.896\times10^{-4}}$ ($\mathbf{+1.6\%}$) & $\mathbf{1.433\times10^{-6}}$ & $\mathbf{1.300\times10^{-3}}$ \\
128--64 (deployed) & $8{,}642$ & $4.129\times10^{-4}$ ($+7.6\%$) & $1.514\times10^{-6}$ & $1.308\times10^{-3}$ \\
...
actually deployed for, $32$--$16$ is the best point estimate on both accuracy
and interval length of every candidate tested, deployed network included --
but attaching Monte Carlo standard errors to the gap, as
Section~\ref{sec:ademp} does throughout, shows it is not resolved: all nine
$\Delta/\mathrm{SE}$ values against the deployed $128$--$64$ fall below $0.6$
in magnitude, nowhere near the $\sim 2$ threshold this paper treats as
resolved elsewhere. Even the nominally worst two-layer candidate, $16$--$8$
at $46\times$ fewer parameters, is measurably worse in only one of nine cells
(the largest-$p$, largest-$J$ cell, where signal is strongest) and tied in the
other eight. Every candidate, deployed network included, beats the GP baseline
...
smaller candidate at this replicate count, and every other number in this
section and in Tables~\ref{tab:mse1D}--\ref{tab:time1D} is reported against it,
so changing it would mean rerunning the paper-scale comparison for a gap this
analysis cannot show is real. The honest conclusion is narrower and, we think,
still useful: this problem does not \emph{require} a network anywhere near
$128$--$64$'s size, downstream performance is flat across more than two orders
of magnitude of parameter count, and a reader retraining this surrogate under
tighter compute or latency constraints can shrink it substantially without
giving up anything the data can distinguish from noise.
