...
\paragraph{Contributions.} This paper makes five contributions. First, we build
...
algorithm. Second, we run a one-dimensional simulation study reproducing
...
credible-interval length, coverage, and wall-clock cost. Third, we run a second
simulation study on the two-stage mutation model that is the original paper's
actual multi-parameter contribution, estimating $(p_1,p_2,\tau)$ jointly.
Fourth, we report, honestly and without tuning it away, that the neural
surrogate's advantage does \emph{not} carry over to that regime, and we pair
that report with a measurement of the data's irreducible noise floor that
explains why. Fifth, we test rather than assume whether a convolutional or
recurrent architecture improves on the architecture-agnostic feedforward
surrogate in that same regime, motivated by where such architectures are known
to help elsewhere in the simulation-based-inference literature, and we report
that neither does.
...
Two conclusions follow, and the second is by far the more useful. The first is
that a network is genuinely required here, and emphatically so: the linear
...
condition rather than a verdict, and the condition is the contribution we would
most like carried forward. Across both studies, the neural surrogate's advantage
...
and it is easy to state and easy to forget. Split-conformal calibration
...
The guarantee does not travel with the number. The two can come apart, and
here they do.
