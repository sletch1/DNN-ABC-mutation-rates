...
(MBP). Its central device is a substitution: inside the ABC-MCMC acceptance
...
\citep{morris2019}. There the neural surrogate is never worse than GPS-ABC on
point-estimation mean squared error in any of nine tested configurations, and
at the largest tested mutation rate it is better by a margin that clearly
exceeds simulation noise (up to $62\%$ lower MSE, a gap of $5$--$8$ Monte Carlo
standard errors); at smaller rates the two are statistically indistinguishable
on point accuracy. The neural surrogate nonetheless returns tighter $95\%$
credible intervals with no loss of empirical coverage in eight of nine
configurations, and achieves near-exact conformal calibration, all at the same
two-to-three-order-of-magnitude speedup over exact ABC-MCMC that the Gaussian
...
design of simulator outputs, then substitute the GP's predictive mean and
variance for the simulator inside the MCMC acceptance step. Every iteration
becomes a GP prediction rather than a simulator call, which is where their
reported two-to-three-order-of-magnitude speedup comes from.
...
\]
This convolution is the mechanism by which the GP's predictive variance enters
the acceptance probability directly, and it sets the standard that any
replacement surrogate must meet. What is required is not merely an accurate
mean but a well-calibrated variance, because the two failure modes are
asymmetric and both corrupt the posterior: an overconfident $\sigma$ inflates
the acceptance rate at parameter values the surrogate does not actually know
well, while an underconfident one over-rejects. A surrogate that predicts the
response perfectly and its own uncertainty badly is not a usable substitute
here.
...
and depth were never themselves swept. We test that directly, holding GELU and
no BatchNorm fixed and shrinking the network from the deployed size down to a
linear control, averaged over three random seeds
(\texttt{network/architecture\_search/benchmark\_capacity.py}).
Table~\ref{tab:capacity1D} also reports each size's downstream ABC-MCMC
accuracy and precision, on the same $p \times J$ grid used throughout
Section~\ref{sec:sim1D_results} at $20$ replicates per cell; the exact-simulator
baseline is not re-run for every candidate, since its cost cannot depend on
the surrogate's size.
...
MSE and 95\% CI length are DNN-ABC on the full $p\times J$ grid, $20$
replicates/cell, one representative seed per row (full detail in
\texttt{results/logs/benchmark\_capacity.md} in the repository).}
\label{tab:capacity1D}
\begin{tabular}{lrrrr}
\toprule
Hidden layers & Parameters & Curve MSE ($\times$ GP) & Downstream MSE & Downstream 95\% CI \\
\midrule
GP (GPS-ABC baseline) & --- & $3.836\times10^{-4}$ (---) & $2.292\times10^{-6}$ & $2.101\times10^{-3}$ \\
128--64 (deployed) & $8{,}642$ & $4.129\times10^{-4}$ ($+7.6\%$) & $9.694\times10^{-7}$ & $1.300\times10^{-3}$ \\
64--32 & $2{,}274$ & $3.957\times10^{-4}$ ($+3.2\%$) & $1.043\times10^{-6}$ & $1.314\times10^{-3}$ \\
\textbf{32--16} & \textbf{626} & $\mathbf{3.896\times10^{-4}}$ ($\mathbf{+1.6\%}$) & $1.236\times10^{-6}$ & $1.382\times10^{-3}$ \\
16--8 & $186$ & $4.508\times10^{-4}$ ($+17.5\%$) & $9.018\times10^{-7}$ & $1.203\times10^{-3}$ \\
16 (1 layer) & $66$ & $4.460\times10^{-4}$ ($+16.3\%$) & $1.207\times10^{-6}$ & $1.388\times10^{-3}$ \\
8 (1 layer) & $34$ & $4.983\times10^{-4}$ ($+29.9\%$) & $1.310\times10^{-6}$ & $1.400\times10^{-3}$ \\
Linear (control) & $4$ & $4.672\times10^{-4}$ ($+21.8\%$) & $1.940\times10^{-6}$ & $1.800\times10^{-3}$ \\
\bottomrule
\end{tabular}
\end{table}

Two things follow, and they point in different directions. On raw curve fit,
capacity is not merely unnecessary above a small threshold -- it is mildly
\emph{counter-productive} at the deployed size: $128$--$64$ has the largest gap
...
actually deployed for, capacity is close to irrelevant: every size from
$34$ to $8{,}642$ parameters -- a $254\times$ range -- lands within a narrow
band on both accuracy and interval length, and all of them beat the GP
baseline by a wide, consistent margin; the single best downstream cell
($16$--$8$) is $46\times$ smaller than the deployed network, and the
difference from $128$--$64$ is not distinguishable from noise at this
replicate count. Only the linear control shows a clear, unambiguous cost,
confirming a nonlinearity is required somewhere, just not much capacity
beyond that.

We do not redeploy a smaller network on the strength of this table: the
deployed $128$--$64$ architecture is not measurably worse downstream than any
smaller candidate, and every other number in this section and in
Tables~\ref{tab:mse1D}--\ref{tab:time1D} is reported against it, so changing it
would mean rerunning the paper-scale comparison for no accuracy gain. The
honest conclusion is narrower and, we think, still useful: this problem does
not \emph{require} a network anywhere near $128$--$64$'s size, and a reader
retraining this surrogate under tighter compute or latency constraints can
shrink it substantially -- by more than an order of magnitude -- without
giving up anything on the metric that actually matters downstream.
...
never worse than GPS-ABC's in nRMSE in any of the nine cells -- but being never
worse is not the same as being reliably better, and that column shows the
comparison is clearly resolved beyond simulation noise in only two of them:
