...
feedforward surrogate, calibrated by split-conformal prediction, that takes
the Gaussian process's place inside an otherwise unchanged ABC-MCMC sampler,
and evaluate it in two simulation studies of a stochastic population-growth
model.

In a one-dimensional benchmark, the network matches or exceeds the Gaussian
process's accuracy and returns tighter credible intervals -- but measuring
posterior coverage directly, rather than inferring it from the surrogate's
regression diagnostics, shows both surrogates under-cover once the
deployment condition departs from the one they were trained at, a general
pitfall for any surrogate queried away from its design. A second,
three-parameter study finds the opposite result: once surrogate error stops
binding, the network confers no advantage, and the comparison is governed by
which parameters the model can identify at all. A comparison finds no
benefit from convolutional or recurrent architectures on this input.

Together, the two studies indicate when a neural surrogate is worth
deploying inside ABC-MCMC, and why its calibration needs verifying inside
the sampler rather than assumed from regression diagnostics alone. Code,
...
\noindent\textbf{Contributions.} This paper makes five contributions. We build
