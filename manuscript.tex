...
\abstract{\citet{lu2023} introduced GPS-ABC, an approximate Bayesian computation (ABC)
...
\For{$t = 1, \dots, T$}
  ...
  \[
  \resizebox{0.95\linewidth}{!}{$\displaystyle\alpha \;=\; \min\!\left(1,\; \frac{\pi(\theta')\,q(\theta_{t-1}\mid\theta')}{\pi(\theta_{t-1})\,q(\theta'\mid\theta_{t-1})}\,\cdot\,\frac{K_\epsilon(\dbar_{\mathrm{obs}}, S(\theta'))}{K_\epsilon(\dbar_{\mathrm{obs}}, S(\theta_{t-1}))}\right)$}
  \]
...
\noindent\textbf{A structural artifact, caught before it could confound this
