...
evaluate it in two simulation studies.
...
surrogates under-cover once deployed away from their training condition. A
second, three-parameter study finds the opposite result for the surrogate
comparison: once surrogate error stops binding, the network confers no
advantage, and the comparison is governed by which parameters the model can
identify at all. There, an amortized neural posterior estimator trained on
the same budget beats every surrogate on the one well-identified parameter,
but fares no better on the two the model cannot pin down. A further
comparison finds no benefit from convolutional or recurrent architectures on
this input.
...
calibration needs verifying directly, not assumed from diagnostics. Code,
data, and a reproducible pipeline accompany the paper.}
...
\citeauthor{lu2023}'s constant-mutation-rate benchmark directly, comparing
four estimators -- method of moments, maximum likelihood, exact ABC-MCMC, and
...
a measurement of the data's irreducible noise floor that explains why. In
that same second study we add an amortized neural posterior estimator (NPE)
trained on the same simulation budget -- scoped here rather than to the
first study because this is the one setting with both a well-identified and
weakly-identified parameter jointly estimated, the contrast that makes the
amortized-vs-surrogate question informative (Section~\ref{sec:sim3D_abc}
gives the full reasoning) -- and find an asymmetry: NPE beats every
surrogate tested on the well-identified parameter, resolved beyond replicate
noise in most comparisons, but does no better than any other method on the
two the model itself cannot pin down. Finally,
...
fresh MCMC run per observed data set. Section~\ref{sec:sim3D_abc} adds an
amortized neural posterior estimator, trained on the same simulation budget,
to the comparison this paper otherwise restricts to the surrogate-inside-MCMC
design, to make that trade-off concrete rather than assumed -- scoped to
Study II rather than Study I for the reason given in the Contributions
paragraph above. The comparison is also motivated by a finding from that literature that bears directly on this
...
\noindent\textbf{Why an amortized estimator is tested here and not in Study I.}
Table~\ref{tab:abc3D} adds a fourth method, an amortized neural posterior
estimator (NPE, Section~\ref{sec:intro}'s Related work paragraph), trained
once on the same $16{,}000$-row budget the surrogates train on and then
queried directly at each replicate's observation with no ABC-MCMC sampler
at all. We test it here rather than in Study I for a reason specific to
this model's structure, not a matter of convenience: Study I has a single
parameter that is well identified by construction, so it cannot pose the
question that makes an amortized-vs-surrogate comparison informative in the
first place -- whether the two approaches behave differently \emph{as a
function of} how identifiable a parameter is. Study II can, because
Section~\ref{sec:sim3D_identifiability} already establishes that $p_2$ is
well identified by the summary statistic while $p_1$ and $\tau$ are not,
within the same jointly estimated posterior. That contrast, not the choice
between ABC-MCMC and NPE in the abstract, is what this comparison is
actually testing, and Study II is the one place in this paper where it can
be tested at all. It is also, not incidentally, the model that is this
paper's own extension of \citeauthor{lu2023} rather than a direct
reproduction of it (Section~\ref{sec:sim3D}), so it is where we test
questions beyond the surrogate-vs-surrogate comparison Study I is built
around -- the architecture-family comparison of Section~\ref{sec:sim3D_arch}
is the other one.
...
(isotropic kernel, untransformed inputs and target). NPE has no acceptance
...
surrogate rather than of the surrogate's own predictions.

A related finding, and one we would place beside the surrogate-quality
result above rather than after it, comes from Section~\ref{sec:sim3D_abc}'s
amortized-estimator comparison. The identifiability ceiling that stops a
better surrogate from helping in Study II turns out not to be specific to
the surrogate-inside-ABC-MCMC design this paper otherwise tests throughout:
an NPE trained directly on simulator draws, with no surrogate step and no
acceptance kernel in between, shows the identical asymmetry. It beats every
surrogate tested, resolved beyond replicate noise in most comparisons, on
$p_2$, the one parameter the model identifies well -- and it does no better
than any other method on $p_1$ or $\tau$, the two the model does not. A
single estimator showing that pattern could be a quirk of the estimator; two
structurally unrelated ones -- a surrogate substituted into an MCMC kernel,
and a density estimator trained once on simulation draws with no kernel at
all -- agreeing on exactly where the ceiling sits is evidence that the
ceiling belongs to the model rather than to either method.

We take this to be a broader methodological point for anyone deploying a
...
Section~\ref{sec:sim3D_abc}'s amortized comparison reinforces this reading
rather than complicating it. An NPE trained directly on simulation draws,
bypassing the surrogate-inside-MCMC design entirely, shows the identical
asymmetry: it beats every surrogate on $p_2$, the one well-identified
parameter, and does no better than any of them on $p_1$ or $\tau$, the two
that are not. That two structurally different estimators -- one a
surrogate substituted into an existing sampler, the other trained once on
simulation draws with no sampler at all -- agree on exactly where the
ceiling sits is stronger evidence than either alone that the ceiling
belongs to the model, not the method, and it is this joint result, on the
two-stage model that is \citeauthor{lu2023}'s own multi-parameter
contribution, that we take to be this paper's central finding.
