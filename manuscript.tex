...
this evenly matched in favour of the smaller model. The deployed network,
effective this revision, is $32$--$16$; Tables~\ref{tab:mse1D}--\ref{tab:time1D}
and every figure in this study are being regenerated against it at the same
paper scale (40 replicates, the full $p\times J$ grid, including the
exact-simulator baseline), and the values below are the outgoing $128$--$64$
run pending that replacement -- retained here, rather than removed early, so
the document is never internally silent about which network a number
describes. The $128$--$64$ row above is retained permanently regardless, as
the architecture the activation and BatchNorm search of
Table~\ref{tab:arch1D} was actually run against, not as the deployed model.
The honest limit on the redeployment decision itself is the one already
stated: the specific margin by which $32$--$16$ leads is not distinguishable
from Monte Carlo noise at this replicate count, so what is established is
that the smaller network is not worse, not that it is provably better.
