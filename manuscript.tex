...
%%=============================================================%%
%% Author block: the author list is still being decided outside %%
%% this conversion task, so \author{} is left empty. The commented %%
%% block below shows the sn-jnl syntax (from sn-article.tex) for a %%
%% 30-second fill-in once the author list is finalised:            %%
%%
...
\title[Deep Neural Network Surrogates for ABC in Markov Branching Processes]{\textbf{Deep Neural Network Surrogates for Approximate Bayesian Computation:\\
A Simulation Study for Mutation-Rate Estimation\\
in Markov Branching Processes}}
\author{}

\abstract{Approximate Bayesian computation (ABC) makes likelihood-free inference
possible for costly stochastic simulators, but its per-iteration cost puts a
full posterior analysis out of reach once the simulator is slow. A standard
remedy substitutes a Gaussian-process surrogate for the simulator inside the
acceptance step of an ABC Markov chain Monte Carlo (MCMC) sampler. We ask
whether a neural-network surrogate can perform the same substitution, and
when doing so helps rather than hurts. We build a heteroscedastic
feedforward surrogate, calibrated by split-conformal prediction, and
evaluate it, alongside an amortized neural posterior estimator trained on
the same budget, in two simulation studies.

In a one-dimensional benchmark, the network matches or exceeds the Gaussian
process's accuracy and returns tighter intervals -- but measuring posterior
coverage directly, not inferring it from regression diagnostics, shows both
surrogates under-cover once deployed away from their training condition. The
amortized estimator ties both on accuracy, covers better, and costs orders
of magnitude less per dataset once trained. A second, three-parameter study
finds the opposite
result for the surrogate comparison: once surrogate error stops binding, the
network confers no advantage, and the comparison is governed by which
parameters the model can identify at all. A further comparison finds no
benefit from convolutional or recurrent architectures on this input.

Together, the studies indicate when a neural surrogate or an amortized
estimator is worth deploying inside or instead of ABC-MCMC, and why
calibration needs verifying directly, not assumed from regression
diagnostics. Code, data, and a reproducible pipeline accompany the paper.}
...
computational budget -- an advantage that does not depend on input dimension
and should, in principle, persist as the parameter and summary-statistic
dimensions grow past the three tested in Study II below, though this paper
tests that only at the two scales it reports rather than along a dimension
sweep. Neither property is free, however. A GP's predictive variance is a direct consequence of the
...
\noindent\textbf{Contributions.} This paper makes six contributions. We build
...
\citeauthor{lu2023}'s constant-mutation-rate benchmark directly, comparing five
estimators -- method of moments, maximum likelihood, exact ABC-MCMC, GPS-ABC,
and an amortized neural posterior estimator (NPE) trained on the same
simulation budget -- against the neural surrogate on estimation accuracy,
credible-interval length, coverage, and wall-clock cost; NPE turns out to be
a further near-exact tie with both surrogates on accuracy, with comparable
or better coverage, at a per-dataset cost that is negligible once trained,
which we take up directly in the trade-off the Related work paragraph below
states and Section~\ref{sec:sim1D_results} measures. We then run a second
...
a measurement of the data's irreducible noise floor that explains why. NPE's
does, but only on the one parameter the model actually identifies well,
where it beats every surrogate tested, resolved beyond replicate noise in
most comparisons -- an asymmetry, not a second general advantage, and we
report it as such. Finally,
...
\noindent\textbf{Related work.} GPS-ABC \citep{meeds2014} is one instance of a
...
The ABC-MCMC comparison below (Section~\ref{sec:sim1D_results}) evaluates
observed data sets simulated at $J \in \{10, 50, 100\}$ cultures, not only the
$J=100$ the ground-truth grid is generated at. Both surrogates are trained
separately for each $J$ rather than once at $J=100$ and queried at every $J$:
for $J \in \{10, 50\}$, a $J$-culture $\dbar$ is reconstructed from the same
underlying simulated data by averaging a random $J$-subset of the $100$
recorded per-culture values (five independent subsets per row, to keep the
resampled training set a reasonable size), so this requires no new simulator
calls. Section~\ref{sec:sim1D_coverage} explains why this matters and reports
what training a single $J=100$ surrogate and querying it away from that
condition actually does to posterior coverage.
...
\noindent\textbf{Why convolutional and recurrent architectures are not tested here.}
...
\noindent\textbf{Is the two-layer 128--64 network actually needed?} Table~\ref{tab:arch1D}
...
(\texttt{network/\allowbreak architecture\_search/\allowbreak benchmark\_capacity.py} for the curve-fit
...
\begin{table*}[t]
\centering
\caption{Study I, Table 1: MSE of $\hat p$ across six estimators; normalized
...
replicates. NPE (Section~\ref{sec:sim1D_results}) is discussed against both
surrogates in the text rather than in a second $\Delta/\mathrm{SE}$ column,
to keep the table to a readable width.}
\label{tab:mse1D}
{\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{rrrrrrrrr}
\toprule
$p$ & $J$ & MOM & MLE & ABC-MCMC & GPS-ABC & DNN-ABC & NPE & $\Delta/\mathrm{SE}$ \\
\midrule
$10^{-4}$ & 10  & 9.36e-9 (0.97) & 1.02e-8 (1.01) & 5.95e-9 (0.77) & 7.01e-9 (0.84) & \textbf{5.25e-9 (0.72)} & 1.54e-8 (1.24) & 0.37 \\
$10^{-4}$ & 50  & 8.68e-9 (0.93) & 9.59e-9 (0.98) & 1.91e-9 (0.44) & 1.98e-9 (0.45) & \textbf{1.77e-9 (0.42)} & 2.07e-9 (0.45) & 0.45 \\
$10^{-4}$ & 100 & 4.18e-9 (0.65) & 4.64e-9 (0.68) & 7.75e-10 (0.28) & 7.95e-10 (0.28) & \textbf{7.68e-10 (0.28)} & 1.36e-9 (0.37) & 0.10 \\
$10^{-3}$ & 10  & 1.38e-6 (1.17) & 1.60e-6 (1.26) & 4.11e-7 (0.64) & 6.46e-7 (0.80) & \textbf{4.58e-7 (0.68)} & 7.88e-7 (0.89) & 0.60 \\
$10^{-3}$ & 50  & 3.21e-7 (0.57) & 3.85e-7 (0.62) & 1.11e-7 (0.33) & \textbf{1.11e-7 (0.33)} & 1.33e-7 (0.36) & 1.39e-7 (0.37) & $-$0.48 \\
$10^{-3}$ & 100 & 2.71e-7 (0.52) & 3.29e-7 (0.57) & 4.29e-8 (0.21) & 4.48e-8 (0.21) & \textbf{4.42e-8 (0.21)} & 4.59e-8 (0.21) & 0.04 \\
$10^{-2}$ & 10  & 3.42e-5 (0.58) & 4.40e-5 (0.66) & 1.67e-5 (0.41) & \textbf{1.77e-5 (0.42)} & 2.30e-5 (0.48) & 1.90e-5 (0.44) & $-$1.02 \\
$10^{-2}$ & 50  & 7.71e-6 (0.28) & 1.17e-5 (0.34) & 7.01e-6 (0.26) & \textbf{5.46e-6 (0.23)} & 9.93e-6 (0.32) & 9.81e-6 (0.31) & $-$1.44 \\
$10^{-2}$ & 100 & 4.01e-6 (0.20) & 6.08e-6 (0.25) & 3.05e-6 (0.17) & \textbf{3.14e-6 (0.18)} & 3.37e-6 (0.18) & 4.95e-6 (0.22) & $-$0.21 \\
\bottomrule
\end{tabular}}
\end{table*}
...
numerically lower than GPS-ABC's in five of the nine cells and higher in the
other four -- but nominally lower is not the same as reliably better, and the
$\Delta/\mathrm{SE}$ column shows that none of these differences, in either
direction, is distinguishable from noise at $R=40$ replicates: the largest
magnitude reached anywhere in the table is $1.44$ ($p=10^{-2}$, $J=50$,
nominally favouring GPS-ABC), short of the $\sim 2$ threshold used throughout
this paper. This is a near-exact statistical tie in every tested cell, consistent with a
GP fit on a comparably sized design already being close to optimal across
this parameter range once both surrogates are trained on the $J$-specific
data described in Section~\ref{sec:sim1D}; Section~\ref{sec:sim1D_coverage}
returns to why training each surrogate to know its own culture count matters
for more than point accuracy. Both surrogates match, and in several
cells slightly improve on, the exact ABC-MCMC baseline, and all ABC-based
methods substantially outperform the classical MOM/MLE estimators --
reproducing \citeauthor{lu2023}'s central finding that ABC dominates the
classical estimators for this model.

NPE's point accuracy joins the same tie: computing its $\Delta/\mathrm{SE}$
against DNN-ABC and against GPS-ABC separately (not tabulated above, to keep
the table to nine columns) gives a maximum magnitude of $1.44$ and $1.47$
respectively across all nine cells, both short of the same $\sim 2$
threshold. This holds even in the one cell where NPE's nominal MSE looks
worst next to the surrogates -- $p=10^{-4}$, $J=10$, roughly $3\times$
DNN-ABC's -- because that cell's Monte Carlo noise is large enough that even
this gap is not distinguishable from it ($\Delta/\mathrm{SE}=1.12$). NPE is
therefore not a weaker baseline on point accuracy at this replicate count;
Section~\ref{sec:sim1D_results} returns to where it differs from the two
surrogates instead: cost and coverage, not accuracy.

\begin{table*}[t]
\centering
\caption{Study I, Table 2: mean length of the 95\% credible interval for
$\hat p$ (NPE's is a posterior credible interval computed the same way, from
direct posterior samples rather than an MCMC chain -- Section~\ref{sec:sim1D_results}).
Bold marks the shorter of GPS-ABC and DNN-ABC.}
\label{tab:ci1D}
\begin{tabular}{rrrrrr}
\toprule
$p$ & $J$ & ABC-MCMC & GPS-ABC & DNN-ABC & NPE \\
\midrule
$10^{-4}$ & 10  & 2.62e-4 & 3.11e-4 & \textbf{2.45e-4} & 4.27e-4 \\
$10^{-4}$ & 50  & 1.56e-4 & 1.50e-4 & \textbf{1.44e-4} & 1.61e-4 \\
$10^{-4}$ & 100 & 1.21e-4 & 1.16e-4 & \textbf{1.16e-4} & 1.48e-4 \\
$10^{-3}$ & 10  & 2.80e-3 & 3.32e-3 & \textbf{2.45e-3} & 3.36e-3 \\
$10^{-3}$ & 50  & 1.43e-3 & 1.66e-3 & \textbf{1.29e-3} & 1.41e-3 \\
$10^{-3}$ & 100 & 9.70e-4 & 1.20e-3 & \textbf{8.81e-4} & 1.08e-3 \\
$10^{-2}$ & 10  & 1.83e-2 & 1.96e-2 & \textbf{1.89e-2} & 2.03e-2 \\
$10^{-2}$ & 50  & 1.16e-2 & 1.74e-2 & \textbf{1.04e-2} & 1.17e-2 \\
$10^{-2}$ & 100 & 7.73e-3 & 1.25e-2 & \textbf{6.80e-3} & 9.19e-3 \\
\bottomrule
\end{tabular}
\end{table*}

DNN-ABC's interval is shorter than GPS-ABC's in all nine cells, $21\%$
tighter on average and up to $46\%$ tighter, without any loss of point
accuracy (Table~\ref{tab:mse1D}) -- though the margin varies widely, from
negligible ($0.1\%$ at $p=10^{-4}$, $J=100$) to substantial ($46\%$ at
$p=10^{-2}$, $J=100$), and against exact ABC-MCMC specifically DNN-ABC is
narrower in seven of nine cells rather than all nine (exact ABC-MCMC is
marginally shorter at $J=10$ for both $p=10^{-4}$ and $p=10^{-2}$). Whether
DNN-ABC's advantage over GPS-ABC is genuine gained precision or overconfidence
is not settled by interval length alone -- Section~\ref{sec:sim1D_coverage}
measures the posterior coverage this comparison needs, and the answer depends
on $J$.

NPE's interval is the widest of the four ABC-based methods in most cells --
wider than ABC-MCMC's in eight of nine and wider than GPS-ABC's in five of
nine -- rather than competing with DNN-ABC on tightness. Whether that is a
weakness or the more honest number is exactly the question interval length
alone cannot answer; Section~\ref{sec:sim1D_results} measures it directly and
finds NPE's coverage tracks its wider intervals more closely than either
surrogate's does.
...
DNN-ABC runs at a flat $\approx 0.13$ seconds per 100 iterations regardless of
$p$ or $J$, because its query cost does not depend on training-set size.
GPS-ABC does not: it costs $\approx 0.12$s at $J=100$ (a $550$-point design)
but $\approx 0.74$s at $J \in \{10, 50\}$ (a $2{,}750$-point design, five
resamples per row -- Section~\ref{sec:sim1D}) -- a $\sim 6\times$ slowdown that
tracks the GP's $O(n)$ per-query cost against the $5\times$ larger design
almost exactly, and the clearest direct demonstration in this paper of the
scaling difference Section~\ref{sec:intro} opens with. Exact ABC-MCMC's cost
explodes with both $p$ and $J$, since it runs the exact simulator $n_s$ times
per iteration. Against exact ABC-MCMC, DNN-ABC is $68$--$2381\times$ faster;
against GPS-ABC it is a statistical tie at $J=100$, as expected, since a GP
over $\sim 550$ points is itself cheap to query, but a real and growing
advantage at $J \in \{10,50\}$ once the $J$-specific design is large enough
for the GP's query cost to matter -- the network's asymptotic query advantage
emerging exactly where design size grows, as Section~\ref{sec:intro}
anticipates, though Study II finds that even a real query-cost advantage does
not translate into better inference once the surrogate is no longer the
binding constraint (Section~\ref{sec:sim3D_abc}). Figure~\ref{fig:posterior1D} shows that the
...
Table~\ref{tab:time1D} has no NPE column because ``seconds per 100 MCMC
iterations'' does not apply to it: NPE has no chain and no per-iteration cost
to divide by, only a one-time training cost and a per-dataset sampling cost,
so Table~\ref{tab:npe1Dtime} reports those directly instead.

\begin{table*}[t]
\centering
\caption{Study I: NPE's cost, reported separately from Table~\ref{tab:time1D}
because it is not an MCMC method. ``Train once'' is paid a single time per
$J$ and shared across every $p$ and replicate at that $J$; ``sample'' is the
marginal cost of a new dataset's posterior, the quantity comparable to
Table~\ref{tab:time1D}'s per-replicate costs.}
\label{tab:npe1Dtime}
\begin{tabular}{rrrr}
\toprule
$p$ & $J$ & train once (s) & sample (ms) \\
\midrule
$10^{-4}$ & 10  & 2.9 & 9.63 \\
$10^{-4}$ & 50  & 2.7 & 3.59 \\
$10^{-4}$ & 100 & 0.6 & 1.08 \\
...
NPE's per-dataset cost -- sub-millisecond to about ten milliseconds -- is
two to three orders of magnitude below even DNN-ABC's already-fast
$\approx 0.13$s per 100 iterations, itself already $68$--$2381\times$ faster
than exact ABC-MCMC; against a full $850$-iteration exact ABC-MCMC replicate
directly (Table~\ref{tab:time1D}'s per-100-iteration cost, scaled up), NPE's
marginal cost per dataset is faster by a factor of roughly $68{,}000\times$
at the cheapest tested cell ($p=10^{-2}$, $J=10$) to roughly $2.4$ million$\times$
at the most expensive ($p=10^{-4}$, $J=100$). This is the amortization
trade-off the Introduction and Related work describe made concrete: the
entire training cost for all three $J$'s combined is about $6$ seconds
(Table~\ref{tab:npe1Dtime}'s ``train once'' column), shared across every $p$
and every one of the $360$ replicates in Tables~\ref{tab:mse1D}--\ref{tab:cov1D},
against the hours the same comparison costs exact ABC-MCMC. What that
amortization buys
in accuracy and coverage is Tables~\ref{tab:mse1D}--\ref{tab:cov1D}'s
finding, not this one: NPE is not more accurate for it, but it is not less
accurate either, and its coverage is the best of the four methods tested
(Section~\ref{sec:sim1D_coverage}).
...
$p$ & $J$ & ABC-MCMC & GPS-ABC & DNN-ABC & NPE \\
\midrule
$10^{-4}$ & 10  & 0.775 (0.066) & 0.825 (0.060) & 0.850 (0.056) & 0.900 (0.047) \\
$10^{-4}$ & 50  & 0.900 (0.047) & 0.875 (0.052) & 0.900 (0.047) & 0.925 (0.042) \\
$10^{-4}$ & 100 & 0.925 (0.042) & 0.950 (0.034) & 0.950 (0.034) & 0.975 (0.025) \\
$10^{-3}$ & 10  & 0.800 (0.063) & 0.875 (0.052) & 0.875 (0.052) & 0.950 (0.034) \\
$10^{-3}$ & 50  & 0.975 (0.025) & 0.975 (0.025) & 0.925 (0.042) & 0.925 (0.042) \\
$10^{-3}$ & 100 & 0.950 (0.034) & 0.975 (0.025) & 0.925 (0.042) & 0.975 (0.025) \\
$10^{-2}$ & 10  & 0.850 (0.056) & 0.875 (0.052) & 0.875 (0.052) & 0.950 (0.034) \\
$10^{-2}$ & 50  & 1.000 (0.000) & 1.000 (0.000) & 0.925 (0.042) & 0.925 (0.042) \\
$10^{-2}$ & 100 & 0.950 (0.034) & 1.000 (0.000) & 0.925 (0.042) & 1.000 (0.000) \\
\bottomrule
\end{tabular}
\end{table*}

A surrogate trained exclusively on $J=100$ summaries has no way to know that
a $J=10$ observation is intrinsically noisier, so its predictive interval
would be calibrated for the wrong noise level whenever $J \ne 100$: querying
the deployed, $J{=}100$-only checkpoints of Section~\ref{sec:arch1D} against
these same $J=10$ replicates, rather than the $J$-specific surrogates
described in Section~\ref{sec:sim1D}, drops GPS-ABC and DNN-ABC coverage
there to $0.43$--$0.53$, roughly half of nominal. Table~\ref{tab:cov1D}
reports the $J$-specific surrogates instead, and two patterns are visible in
it. First, training each surrogate to know its own culture count recovers
most, though not all, of that shortfall: at $J=10$, GPS-ABC and DNN-ABC now
cover at $0.825$--$0.875$ -- $1$--$3$ Monte Carlo standard errors below the
nominal $0.95$, a real residual gap, but a small one next to the roughly
$0.45$ gap the $J$-blind surrogate leaves -- and both reach or exceed nominal
by $J=100$. Neither surrogate is consistently the more under-covering of the
two at any $J$ now; Table~\ref{tab:ci1D}'s interval-length advantage for
DNN-ABC is accordingly better evidence of genuine precision at this
replicate count than it would be if one surrogate's intervals were also the
least calibrated. Second, and separately, exact ABC-MCMC itself is not at
nominal at $J=10$ ($0.775$--$0.850$) despite calling the true simulator at
the correct $J$ on every iteration, and is not obviously better calibrated
there than either $J$-specific surrogate -- a property of the ABC acceptance
kernel (the synthetic-likelihood construction of Algorithm~\ref{alg:abcmcmc})
at this culture count, not of any surrogate, and not explained by the
$J$-mismatch mechanism above.

NPE's coverage is the best-calibrated of the four methods at nearly every
$J$: never below $0.900$ anywhere in the table (against GPS-ABC's low of
$0.825$ and DNN-ABC's $0.850$), at or above the nominal $0.95$ in five of
nine cells, and closest to nominal in six. This is consistent with, and
mechanistically explained by, Table~\ref{tab:ci1D}'s finding that NPE's
credible intervals are also usually the widest of the four: an amortized
posterior estimator trained directly on simulator draws, with no ABC
acceptance kernel or tolerance $\epsilon$ standing between the training
signal and the reported interval, does not inherit the small-$J$ overconfidence
mechanism identified above for a surrogate plugged into that kernel. This is
the calibration side of the accuracy-vs-cost trade-off the Introduction and
Related work raise: on this evidence, NPE is not a weaker method here, and
its coverage advantage plus negligible per-dataset cost (Table~\ref{tab:npe1Dtime}) argue
for it specifically where calibrated uncertainty at deployment time matters
more than squeezing out the last few percent of point accuracy -- while
GPS-ABC and DNN-ABC remain preferable where an existing ABC-MCMC workflow,
an explicit tolerance parameter, or a very small simulation budget is the
binding constraint, as Section~\ref{sec:intro} anticipates.

The remaining gap at $J=10$ is worth being precise about rather than
explaining away. Split-conformal calibration (Section~\ref{sec:sim1D_calib})
is a marginal guarantee, averaged over the surrogate's whole training
distribution; it does not guarantee the surrogate's predictive interval is
correctly sized at every individual $\theta$, and the $J=10$ training design
is the smallest and noisiest of the three (Section~\ref{sec:sim1D}), which is
the more likely source of the residual gap than any single obvious defect.
\textbf{The practical lesson is the same regardless of that residual}: a
surrogate's interval-length advantage in Table~\ref{tab:ci1D} is not by
itself evidence of equal or better calibration, and training a surrogate
that knows the deployment condition it will actually be queried at is
necessary, though on this evidence not always fully sufficient, for its ABC
posterior to be trustworthy in a way no regression diagnostic computed at
the training condition alone can reveal.
...
\noindent\textbf{The summary statistic is the fourth root.} \citeauthor{lu2023} use
...
\noindent\textbf{This is not \citeauthor{lu2023}'s parameter regime.} It should be
...
\noindent\textbf{Does the curve-fit tie survive into the downstream task?} Table~\ref{tab:capacity3D}
...
\noindent\textbf{Heteroscedasticity.} The second output head is not optional here. The
...
\noindent\textbf{Mixed activations across layers.}
...
\noindent\textbf{The deployed activation.} GELU into tanh is the best point estimate
...
\noindent\textbf{Two Gaussian-process baselines, and why both are reported.} Our GP
...
(isotropic kernel, untransformed inputs and target). NPE (Section~\ref{sec:sim1D_results})
has no acceptance step, so its rate column reads ``---''.}
\label{tab:abc3D}
\begin{tabular}{llrrrr}
\toprule
...
DNN-ABC & $p_1$   & 1.407 & 3.43e-2 & 0.938 & 0.143 \\
        ...
        & $\tau$  & 1.753 & 8.64 & 1.000 & \\
\midrule
NPE & $p_1$   & 1.373 & 3.10e-2 & 1.000 & --- \\
        & $p_2$   & \textbf{0.089} & 2.89e-2 & 1.000 & \\
        & $\tau$  & 1.425 & 8.77 & 1.000 & \\
\bottomrule
\end{tabular}
\end{table*}
...
\emph{NPE is the one method that does favour an amortized estimator here, and
only on the well-identified parameter.} On $p_2$, NPE's RMSE ($0.089$) beats
every surrogate at every truth tested -- never the reverse -- and the gap is
resolved at the $2$-MCSE level in seven of the nine surrogate comparisons
(all three against the reference GP, two of three each against the
strengthened GP and DNN-ABC; the remaining two are ties, never a surrogate
win). On $p_1$ and $\tau$, the weakly identified parameters, NPE is a mix of
ties and occasional wins and losses against each GP variant, consistent with
Section~\ref{sec:sim3D_identifiability}'s point that no method can extract
information the summary statistic does not carry: an amortized estimator
trained directly on simulator draws is not exempt from that limit, and
Table~\ref{tab:abc3D} does not show it beating the identifiability ceiling
either. The asymmetry is the finding: where the model actually identifies a
parameter, learning the posterior directly from simulations outperformed
substituting a surrogate into the same ABC-MCMC kernel Study I uses,
resolved beyond replicate noise; where it does not, no method -- amortized or
not -- does better than any other. Training cost and per-dataset sampling
cost are negligible here for the same reason as Study I (seconds to train,
milliseconds per dataset); the finding is about accuracy, not speed, and is
...
\noindent\textbf{Setup.} We train a 1-D CNN (two convolutional layers, kernel width
...
\noindent\textbf{Scope and limitations.} This paper is a simulation study
...
\noindent\textbf{Code and data availability.} All code, data, trained model
...
\noindent\textbf{Computational environment.} All code is Python (minimum versions
...
\noindent\textbf{Instructions for reproduction.} Each study ships a single entry
...
[PLACEHOLDER: author contributions will be stated here once the author list
is finalised.]
