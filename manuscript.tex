...
this evenly matched in favour of the smaller model. Every number in
Tables~\ref{tab:mse1D}--\ref{tab:time1D} and every figure in this study is
therefore reported for the $32$--$16$ network, retrained from the same data
splits and re-evaluated at the same paper scale; the $128$--$64$ row above is
retained as the architecture the activation and BatchNorm search of
Table~\ref{tab:arch1D} was actually run against, not as the deployed model.
The honest limit on this conclusion is the one already stated: the specific
margin by which $32$--$16$ leads is not itself distinguishable from Monte
Carlo noise at this replicate count, so what is established is that the
smaller network is not worse, not that it is provably better.
