...
#
#   bash Models/3D/run_on_server.sh              # full run (2000 x 10)
#   bash Models/3D/run_on_server.sh 20 2         # smoke test (20 x 2)
...
ssh "$HOST" "mkdir -p ~/$REMOTE/RCode ~/$REMOTE/Models/3D/data"
...
scp "$HOST:~/$REMOTE/Models/3D/data/slow_data_3D.csv" \
    "$HOST:~/$REMOTE/Models/3D/data/convention_check_3D.csv" \
    "$LOCAL_3D/data/"
